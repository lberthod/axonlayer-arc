<template>
  <div class="bg-slate-800 rounded-xl border-2 border-indigo-500/20 p-8 mb-6">
    <div class="max-w-3xl mx-auto">
      ...
      <div class="bg-slate-800 rounded-lg p-6 mb-4 border-2"
        :class="step >= 1 ? 'border-emerald-500/30 bg-emerald-950/20' : 'border-indigo-500/20'">
        <div class="flex items-start gap-4">
          <div class="flex-shrink-0">
            <div
              :class="['w-10 h-10 rounded-full flex items-center justify-center font-bold text-white', step >= 1 ? 'bg-emerald-500' : 'bg-slate-600']">
              {{ step >= 1 ? '✓' : '1' }}
            </div>
          </div>
          <div class="flex-1">
            ...
            <div v-if="!walletData">
              <button @click="createWallet" :disabled="creatingWallet"
            ...
            <div v-else class="space-y-4">
              ...
              <div class="bg-slate-800 rounded-lg p-4 border border-indigo-500/20">
                ...
                <div class="flex items-center gap-2 mb-3">
                  <code
                    class="flex-1 text-sm font-mono bg-slate-900 border border-slate-600 text-slate-300 p-2 rounded break-all">{{ walletData.address }}</code>
              ...
              <div class="bg-red-950/20 rounded-lg p-4 border border-red-900/50">
                ...
                <div class="flex items-center gap-2">
                  <code
                    class="flex-1 text-xs font-mono bg-slate-900 border border-slate-600 text-slate-300 p-2 rounded break-all"
      ...
      <!-- Step 2: Fund Wallet -->
      <div class="bg-slate-800 rounded-lg p-6 border-2"
        :class="step >= 2 ? 'border-emerald-500/30 bg-emerald-950/20' : 'border-indigo-500/20'">
        <div class="flex items-start gap-4">
          <div class="flex-shrink-0">
            <div
              :class="['w-10 h-10 rounded-full flex items-center justify-center font-bold text-white', step >= 2 ? 'bg-emerald-500' : 'bg-slate-600']">
              {{ step >= 2 ? '✓' : '2' }}
            </div>
          </div>
          <div class="flex-1">
            ...
            <div v-if="walletData" class="space-y-4">
              ...
              <div v-if="currentBalance !== null"
                class="p-4 bg-emerald-950/20 rounded-lg border-2 border-emerald-900/50">
                ...
                <p v-if="currentBalance > 0" class="text-xs text-emerald-400 font-semibold">✅ Wallet funded! Ready to
                  launch missions!</p>
      ...
      <!-- Success State -->
      <div v-if="step >= 2 && currentBalance > 0"
        class="mt-6 p-6 bg-emerald-950/20 rounded-lg border-2 border-emerald-900/50 text-center">
        ...
        <button @click="$emit('setup-complete')"
      ...
      <div class="mt-6 p-4 bg-red-950/20 rounded-lg border border-red-900/50">
        <p class="text-sm text-red-400">
          ...
          Store them in a secure location (password manager, hardware wallet, etc.). Axonlayer will never ask for your
          private key.
        </p>
      </div>
    </div>
